...
class Context
{
  ...
  public:
    IResultCapture *getResultCapture() const
    {
...
class IResultCapture
{
  public:
    ...
    virtual IGeneratorTracker *createGeneratorTracker(StringRef generatorName, SourceLineInfo lineInfo,
    ...
    virtual void handleMessage(AssertionInfo const &info, ResultWas::OfType resultType, StringRef message,
                               AssertionReaction &reaction) = 0;
    virtual void handleUnexpectedExceptionNotThrown(AssertionInfo const &info, AssertionReaction &reaction) = 0;
    virtual void handleUnexpectedInflightException(AssertionInfo const &info, std::string &&message,
                                                   AssertionReaction &reaction) = 0;
...
template <class Duration> struct BenchmarkStats
{
    ...
    template <typename Duration2> operator BenchmarkStats<Duration2>() const
    {
        ...
        return {
            info, CATCH_MOVE(samples2), mean, standardDeviation, outliers, outlierVariance,
        };
    }
};
...
struct Chronometer
{
  public:
    ...
    Chronometer(Detail::ChronometerConcept &meter, int repeats_) : impl(&meter), repeats(repeats_)
    {
    }
...
TimingOf<Clock, Fun, run_for_at_least_argument_t<Clock, Fun>> run_for_at_least(ClockDuration<Clock> how_long,
                                                                               const int initial_iterations, Fun &&fun)
{
...
double weighted_average_quantile(int k, int q, std::vector<double>::iterator first,
                                 std::vector<double>::iterator aLast);

OutlierClassification classify_outliers(std::vector<double>::const_iterator first,
                                        std::vector<double>::const_iterator aLast);

double mean(std::vector<double>::const_iterator first, std::vector<double>::const_iterator aLast);

template <typename Estimator>
sample jackknife(Estimator &&estimator, std::vector<double>::iterator first, std::vector<double>::iterator aLast)
{
    auto n = static_cast<size_t>(aLast - first);
    auto second = first;
    ++second;
    sample results;
    results.reserve(n);

    for (auto it = first; it != aLast; ++it)
    {
        std::iter_swap(it, first);
        results.push_back(estimator(second, aLast));
    }
...
template <typename Estimator>
Estimate<double> bootstrap(double confidence_level, std::vector<double>::iterator first,
                           std::vector<double>::iterator aLast, sample const &resample, Estimator &&estimator)
{
    auto n_samples = aLast - first;

    double point = estimator(first, aLast);
    // Degenerate case with a single sample
    if (n_samples == 1)
        return {point, point, point, confidence_level};

    sample jack = jackknife(estimator, first, aLast);
...
bootstrap_analysis analyse_samples(double confidence_level, unsigned int n_resamples,
                                   std::vector<double>::iterator first, std::vector<double>::iterator aLast);
} // namespace Detail
} // namespace Benchmark
} // namespace Catch
...
template <typename Duration> struct SampleAnalysis
{
    ...
    template <typename Duration2> operator SampleAnalysis<Duration2>() const
    {
        ...
        return {
            CATCH_MOVE(samples2), mean, standard_deviation, outliers, outlier_variance,
        };
    }
};
} // namespace Benchmark
...
SampleAnalysis<Duration> analyse(const IConfig &cfg, Environment<Duration>, Iterator first, Iterator aLast)
{
    if (!cfg.benchmarkNoAnalysis())
    {
        std::vector<double> samples;
        samples.reserve(static_cast<size_t>(aLast - first));
        for (auto current = first; current != aLast; ++current)
        {
    ...
    else
    {
        ...
        int i = 0;
        for (auto it = first; it < aLast; ++it, ++i)
        {
...
struct Benchmark
{
    ...
    template <class FUN>
    Benchmark(std::string &&benchmarkName, FUN &&func) : fun(CATCH_MOVE(func)), name(CATCH_MOVE(benchmarkName))
    {
    }
    ...
    template <typename Clock = default_clock> void run()
    {
        ...
        CATCH_TRY
        {
            ...
            BenchmarkInfo info{CATCH_MOVE(name),           plan.estimated_duration.count(),
            ...
            BenchmarkStats<FloatDuration<Clock>> stats{CATCH_MOVE(info),  CATCH_MOVE(analysis.samples),
...
std::string rangeToString(InputIterator first, Sentinel aLast)
{
    ReusableStringStream rss;
    rss << "{ ";
    if (first != aLast)
...
struct SourceLineInfo
{
    ...
    constexpr SourceLineInfo(char const *_file, std::size_t _line) noexcept : file(_file), line(_line)
    {
    }
...
template <typename T> class Optional
{
  public:
    ...
    Optional(Optional &&_other) : nullableValue(_other ? new(storage) T(CATCH_MOVE(*_other)) : nullptr)
    {
    }
...
struct StreamEndStop
{
    ...
    template <typename T> constexpr friend T const &operator+(T const &value, StreamEndStop)
    {
        return value;
    }
};
...
#define INTERNAL_CATCH_CAPTURE(varName, macroName, ...)                                                                \
    Catch::Capturer varName(macroName##_catch_sr, CATCH_INTERNAL_LINEINFO, Catch::ResultWas::Info,                     \
...
template <typename T = void> class BasicResult : public ResultValueBase<T>
{
  ...
  protected:
    ...
    BasicResult(ResultType type, std::string &&message) : ResultValueBase<T>(type), m_errorMessage(CATCH_MOVE(message))
    {
...
class Opt : public Detail::ParserRefImpl<Opt>
{
  ...
  public:
    template <typename LambdaT>
    explicit Opt(LambdaT const &ref) : ParserRefImpl(std::make_shared<Detail::BoundFlagLambda<LambdaT>>(ref))
    {
    }
...
class Args
{
  ...
  public:
    ...
    std::string const &exeName() const
    {
        return m_exeName;
    }
};
...
#define CATCH_INTERNAL_DEFINE_EXPRESSION_EQUALITY_OPERATOR(id, op)                                                     \
    ...
    friend auto operator op(ExprLhs &&lhs, RhsT &&rhs) -> std::enable_if_t<                                            \
        Detail::conjunction<Detail::is_##id##_comparable<LhsT, RhsT>,                                                  \
                            Detail::negation<std::is_arithmetic<std::remove_reference_t<RhsT>>>>::value,               \
    ...
    template <typename RhsT>                                                                                           \
    friend auto operator op(ExprLhs &&lhs, RhsT rhs) -> std::enable_if_t<                                              \
        Detail::conjunction<Detail::is_##id##_comparable<LhsT, RhsT>, std::is_arithmetic<RhsT>>::value,                \
    ...
    template <typename RhsT>                                                                                           \
    friend auto operator op(ExprLhs &&lhs, RhsT rhs) -> std::enable_if_t<                                              \
        Detail::conjunction<Detail::negation<Detail::is_##id##_comparable<LhsT, RhsT>>,                                \
...
#define CATCH_INTERNAL_DEFINE_EXPRESSION_OPERATOR(op)                                                                  \
    template <typename RhsT>                                                                                           \
    friend auto operator op(ExprLhs &&lhs, RhsT &&rhs)                                                                 \
        -> std::enable_if_t<!std::is_arithmetic<std::remove_reference_t<RhsT>>::value, BinaryExpr<LhsT, RhsT const &>> \
    {                                                                                                                  \
        return {static_cast<bool>(lhs.m_lhs op rhs), lhs.m_lhs, #op##_sr, rhs};                                        \
    }                                                                                                                  \
    template <typename RhsT>                                                                                           \
    friend auto operator op(ExprLhs &&lhs, RhsT rhs)                                                                   \
        -> std::enable_if_t<std::is_arithmetic<RhsT>::value, BinaryExpr<LhsT, RhsT>>                                   \
    {                                                                                                                  \
        return {static_cast<bool>(lhs.m_lhs op rhs), lhs.m_lhs, #op##_sr, rhs};                                        \
    }
...
#define INTERNAL_CATCH_TEST(macroName, resultDisposition, ...)                                                         \
    ...
    { /* NOLINT(bugprone-infinite-loop) */                                                                             \
        ...
        INTERNAL_CATCH_REACT(catchAssertionHandler)                                                                    \
             ...
             (false) && static_cast<const bool &>(!!(__VA_ARGS__))) // the expression here is never evaluated at runtime
...
#define INTERNAL_CATCH_SECTION(...)                                                                                    \
    ...
    if ([[maybe_unused]] int catchInternalPreviousSectionHint = catchInternalSectionHint,                              \
...
{
    AutoReg(Detail::unique_ptr<ITestInvoker> invoker, SourceLineInfo const &lineInfo, StringRef classOrMethod,
...
#define INTERNAL_CATCH_TESTCASE(...) INTERNAL_CATCH_TESTCASE2(INTERNAL_CATCH_UNIQUE_NAME(dummyFunction))
        ...
        {                                                                                                              \
            ...
            (void)expander{                                                                                            \
                (Catch::AutoReg(                                                                                       \
                     ...
                     Catch::NameAndTags{Name " - " INTERNAL_CATCH_STRINGIZE(TmplList) " - " + std::to_string(index),   \
                                        Tags}),                                                                        \
                 index++)...}; /* NOLINT */                                                                            \
        }                                                                                                              \
    };                                                                                                                 \
...
#define INTERNAL_CATCH_TEMPLATE_LIST_TEST_CASE_METHOD_2(TestNameClass, TestName, ClassName, Name, Tags, TmplList)      \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            (void)expander{                                                                                            \
                (Catch::AutoReg(                                                                                       \
                     Catch::makeTestInvoker(&TestName<Types>::test), CATCH_INTERNAL_LINEINFO, #ClassName##_catch_sr,   \
                     Catch::NameAndTags{Name " - " INTERNAL_CATCH_STRINGIZE(TmplList) " - " + std::to_string(index),   \
                                        Tags}),                                                                        \
                 index++)...}; /* NOLINT */                                                                            \
        }                                                                                                              \
...
class ExceptionTranslatorRegistrar
{
    ...
    {
      public:
        ExceptionTranslator(std::string (*translateFunction)(T const &)) : m_translateFunction(translateFunction)
        {
        }
    ...
    typename decltype(generatorExpression())::type
{
    ...
    if (!tracker)
    {
        tracker = createGeneratorTracker(generatorName, lineInfo,
...
template <typename T> class TakeGenerator final : public IGenerator<T>
{
  ...
  public:
    TakeGenerator(size_t target, GeneratorWrapper<T> &&generator) : m_generator(CATCH_MOVE(generator)), m_target(target)
    {
...
{
    static_assert(!std::is_same<T, char>::value && !std::is_same<T, int8_t>::value &&
                      !std::is_same<T, uint8_t>::value && !std::is_same<T, signed char>::value &&
...
template <typename T> class IteratorGenerator final : public IGenerator<T>
{
  ...
  public:
    ...
    IteratorGenerator(InputIterator first, InputSentinel aLast) : m_elems(first, aLast)
    {
        if (m_elems.empty())
        {
...
template <typename T> constexpr bool empty(std::initializer_list<T> list) noexcept
{
    return list.size() > 0;
}
...
std::enable_if_t<!std::is_same<ForwardIter, Sentinel>::value, std::ptrdiff_t> sentinel_distance(ForwardIter iter,
...
template <typename ForwardIter> std::ptrdiff_t sentinel_distance(ForwardIter first, ForwardIter aLast)
{
    return std::distance(first, aLast);
}

template <typename ForwardIter1, typename Sentinel1, typename ForwardIter2, typename Sentinel2, typename Comparator>
bool check_element_counts(ForwardIter1 first_1, const Sentinel1 end_1, ForwardIter2 first_2, const Sentinel2 end_2,
                          Comparator cmp)
{
    auto cursor = first_1;
    while (cursor != end_1)
...
template <typename ForwardIter1, typename Sentinel1, typename ForwardIter2, typename Sentinel2, typename Comparator>
bool is_permutation(ForwardIter1 first_1, const Sentinel1 end_1, ForwardIter2 first_2, const Sentinel2 end_2,
                    Comparator cmp)
{
...
struct NameAndLocationRef
{
    ...
    constexpr NameAndLocationRef(StringRef name_, SourceLineInfo location_) : name(name_), location(location_)
    {
    }
...
class RunContext final : public IResultCapture
{
  ...
  public: // IResultCapture
    ...
    void handleMessage(AssertionInfo const &info, ResultWas::OfType resultType, StringRef message,
...
class pluralise
{
  ...
  public:
    constexpr pluralise(std::uint64_t count, StringRef label) : m_count(count), m_label(label)
    {
    }
  ...
  public:
    MatchExpr(ArgT &&arg, MatcherT const &matcher)
        : ITransientExpression{true, matcher.match(arg)}, // not forwarding arg here on purpose
          m_arg(CATCH_FORWARD(arg)), m_matcher(matcher)
    {
    }
...
template <typename... MatcherTs> class MatchAllOfGeneric final : public MatcherGenericBase
{
  public:
    ...
    friend std::enable_if_t<is_matcher<MatcherRHS>::value, MatchAllOfGeneric<MatcherTs..., MatcherRHS>> operator&&(
        MatchAllOfGeneric<MatcherTs...> &&lhs, MatcherRHS const &rhs)
    {
    ...
    friend std::enable_if_t<is_matcher<MatcherLHS>::value, MatchAllOfGeneric<MatcherLHS, MatcherTs...>> operator&&(
        MatcherLHS const &lhs, MatchAllOfGeneric<MatcherTs...> &&rhs)
    {
...
template <typename... MatcherTs> class MatchAnyOfGeneric final : public MatcherGenericBase
{
  public:
    ...
    template <typename MatcherRHS>
    friend std::enable_if_t<is_matcher<MatcherRHS>::value, MatchAnyOfGeneric<MatcherTs..., MatcherRHS>> operator||(
        MatchAnyOfGeneric<MatcherTs...> &&lhs, MatcherRHS const &rhs)
    {
    ...
    template <typename MatcherLHS>
    friend std::enable_if_t<is_matcher<MatcherLHS>::value, MatchAnyOfGeneric<MatcherLHS, MatcherTs...>> operator||(
        MatcherLHS const &lhs, MatchAnyOfGeneric<MatcherTs...> &&rhs)
    {
...
template <typename Matcher> NoneMatchMatcher<Matcher> NoneMatch(Matcher &&matcher)
{
    return {CATCH_FORWARD(matcher)};
}
...
std::enable_if_t<!Detail::is_matcher<RangeLike>::value, RangeEqualsMatcher<RangeLike, std::equal_to<>>> RangeEquals(
    RangeLike &&range)
{
 ...
 */
template <typename RangeLike>
std::enable_if_t<!Detail::is_matcher<RangeLike>::value, UnorderedRangeEqualsMatcher<RangeLike, std::equal_to<>>>
